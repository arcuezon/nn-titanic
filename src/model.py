...
def L_layer_NN(X, Y, layer_dims={}, save_params=True, load_params=False,
               grads_momentum=True, rate_decay=False, learning_rate=0.01,
               epochs=1000, decay_rate = 0.1):
    ...
    for epoch in range(1, epochs):
        ...
        if(rate_decay):
            ...
            learning_rate = np.divide(1, 1 + decay_rate * epoch) * learning_rate
        ...
        if(grads_momentum):
            ...
            L = len(caches)
            for l in range(1, L + 1):
                vgrads["dW" + str(l)] = beta * vgrads["dW" +
                                                      str(l)] + (1 - beta) * grads["dW" + str(l)]
                vgrads["db" + str(l)] = beta * vgrads["db" +
                                                      str(l)] + (1 - beta) * grads["db" + str(l)]
